...
-- 3) Public listing photo storage
insert into storage.buckets (id,name,public)
values ('renthub-images','renthub-images',true)
on conflict (id) do update set public=true;

-- Do NOT ALTER storage.objects here. Supabase owns this managed table and
-- normally has RLS enabled already. The SQL Editor role is not the owner.
...
create policy renthub_images_public_read
on storage.objects for select
using (bucket_id='renthub-images');

create policy renthub_images_anon_insert
on storage.objects for insert
to anon with check (bucket_id='renthub-images');

create policy renthub_images_anon_update
on storage.objects for update
to anon using (bucket_id='renthub-images')
with check (bucket_id='renthub-images');

create policy renthub_images_anon_delete
