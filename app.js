/* RentHub public site script. All persistent records and photos are stored in Supabase. */
...
function applicantInitials(first, surname){
  const parts=[String(first||'').trim(), String(surname||'').trim()].filter(Boolean);
  return parts.map(x=>x[0]).join('').toUpperCase().replace(/[^A-Z]/g,'').slice(0,3) || 'APP';
}
function applicantReference(phone, first, surname){
  const digits=String(phone||'').replace(/\D/g,'');
  return `${digits}@${applicantInitials(first,surname)}`;
}
...
function roomListings(){
  return (Array.isArray(getData().rooms)?getData().rooms:[]).filter(r=>r&&r.id).map(r=>{
    const p=r.propertyId?getData().properties.find(x=>x.id===r.propertyId):null;
...
function relatedProperty(r){return r.propertyId?getData().properties.find(p=>p.id===r.propertyId):null}
...
function startApplicationStandalone(id){
  ...
  $('#applicationForm').onsubmit=e=>{e.preventDefault(); const f=new FormData(e.target); const phone=String(f.get('phone')).trim(); const tenantNo=String(f.get('tenantNumber')).trim(); if(!/^\+?[0-9 ()-]{7,20}$/.test(phone)){alert('Enter a valid phone number.');return} if(getData().applications.some(a=>a.roomId===r.id&&a.tenantNumber===tenantNo&&['RECEIVED','APPROVED'].includes(a.status))){alert('An active application already exists for this tenant and room.');return} const first=String(f.get('first')).trim(), surname=String(f.get('surname')).trim(), email=String(f.get('email')).trim(); const applicationNumber=applicantReference(phone,first,surname); const a={id:uid('APP'),applicationNumber,roomId:r.id,propertyId:r.propertyId||null,tenantNumber:tenantNo,tenant:{first,surname,phone,email},message:String(f.get('message')).trim(),status:'RECEIVED',createdAt:new Date().toISOString()}; getData().applications.unshift(a); Store.save(); Store.log('New tenant application',`${a.tenant.first} ${a.tenant.surname} — ${r.name}`); $('#applicationResult').innerHTML=`<div class="chip" style="margin-top:12px">Application submitted. Reference: <b>${escapeHtml(a.applicationNumber)}</b>. Save this reference to check your status.</div>`; e.target.reset(); };
}
...
$("#statusForm").onsubmit = e => {
  e.preventDefault();
  const token = $("#statusToken").value.trim().toLowerCase(); const a = getData().applications.find(x => String(x.applicationNumber||x.id||'').toLowerCase()===token);
...
$("#landlordForm").onsubmit = e => {
  ...
  const phone=String(f.get('phone')||'').trim(); if(!/^\+?[0-9 ()-]{7,20}$/.test(phone)){alert('Enter a valid phone number.');return} if(getData().landlords.some(x=>String(x.phone||'').replace(/\D/g,'')===phone.replace(/\D/g,''))){alert('A landlord application already exists for this phone number.');return} const firstName=String(f.get('name')).trim(); const parts=firstName.split(/\s+/).filter(Boolean); const ref=applicantReference(phone,parts[0]||'',parts.slice(1).join(' ')||parts[0]||''); const rec={ id: uid('L'), applicationNumber:ref, name:firstName, company:String(f.get('company')).trim(), phone, whatsapp:String(f.get('whatsapp')).trim(), email:String(f.get('email')).trim(), address:String(f.get('address')).trim(), property:String(f.get('property')).trim(), ownership:String(f.get('ownership')).trim(), status:'PENDING' }; getData().landlords.push(rec); Store.save(); Store.log('New landlord application',rec.name); alert('Your landlord application has been submitted. Reference: '+rec.applicationNumber); e.target.reset();
};
$("#agentForm").onsubmit = e => {
  e.preventDefault();
  const f = new FormData(e.target);
  const phone=String(f.get('phone')||'').trim(); if(!/^\+?[0-9 ()-]{7,20}$/.test(phone)){alert('Enter a valid phone number.');return} if(getData().agents.some(x=>String(x.phone||'').replace(/\D/g,'')===phone.replace(/\D/g,''))){alert('An agent application already exists for this phone number.');return} const firstName=String(f.get('name')).trim(); const parts=firstName.split(/\s+/).filter(Boolean); const ref=applicantReference(phone,parts[0]||'',parts.slice(1).join(' ')||parts[0]||''); const rec={ id: uid('A'), applicationNumber:ref, name:firstName, business:String(f.get('business')).trim(), phone, whatsapp:String(f.get('whatsapp')).trim(), email:String(f.get('email')).trim(), address:String(f.get('address')).trim(), type:String(f.get('type')), reason:String(f.get('reason')).trim(), status:'PENDING' }; getData().agents.push(rec); Store.save(); Store.log('New agent application',rec.name); alert('Your Agent application has been submitted. Reference: '+rec.applicationNumber); e.target.reset();
};
$("#contactBtn").onclick = () => alert(getData().contact);
$("#brandName").textContent = getData().brandName;
$("#year").textContent = new Date().getFullYear();
document.title = getData().brandName;
initFilters(); render();
