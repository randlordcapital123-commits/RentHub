...
$("#statusForm").onsubmit = e => {
  e.preventDefault();
  const token = $("#statusToken").value.trim().toLowerCase(); const a = data.applications.find(x => String(x.applicationNumber||x.id||'').toLowerCase()===token);
...
$("#agentForm").onsubmit = e => {
  e.preventDefault();
  const f = new FormData(e.target);
  const phone=String(f.get('phone')||'').trim(); if(!/^\+?[0-9 ()-]{7,20}$/.test(phone)){alert('Enter a valid phone number.');return} if(data.agents.some(x=>String(x.phone||'').replace(/\D/g,'')===phone.replace(/\D/g,''))){alert('An agent application already exists for this phone number.');return} const firstName=String(f.get('name')).trim(); const parts=firstName.split(/\s+/).filter(Boolean); const ref=applicantReference(phone,parts[0]||'',parts.slice(1).join(' ')||parts[0]||''); const rec={ id: uid('A'), applicationNumber:ref, name:firstName, business:String(f.get('business')).trim(), phone, whatsapp:String(f.get('whatsapp')).trim(), email:String(f.get('email')).trim(), address:String(f.get('address')).trim(), type:String(f.get('type')), reason:String(f.get('reason')).trim(), status:'PENDING' }; data.agents.push(rec); Store.save(); Store.log('New agent application',rec.name); alert('Your Agent application has been submitted. Reference: '+rec.applicationNumber); e.target.reset();
};
