...
const Store = (() => {
  ...
  async function putImage(blob){
    // Always optimize browser photos before uploading. This keeps the database small and
    // prevents a phone camera image from making the marketplace slow.
    let uploadBlob = blob;
    try {
    ...
    if(SUPA_ENABLED){
      try {
        const path=`rooms/${Date.now()}_${Math.random().toString(36).slice(2,10)}.jpg`;
        await restRequest(`/storage/v1/object/renthub-images/${path}`, {
          method:'POST',
          headers:{'Content-Type':'image/jpeg','x-upsert':'false','Cache-Control':'public,max-age=31536000,immutable'},
          body:uploadBlob
        });
        return `${SUPA_URL}/storage/v1/object/public/renthub-images/${path}`;
      } catch(e){
        console.error('Supabase image upload failed:',e);
        throw new Error('Image upload failed. Check the renthub-images Storage policies in Supabase.');
      }
    }
    const key=uid("IMG"),ref="local:"+key;
    try{const db=await openImageDB();await new Promise((res,rej)=>{const tx=db.transaction(IMAGE_STORE,"readwrite");tx.objectStore(IMAGE_STORE).put(uploadBlob,key);tx.oncomplete=res;tx.onerror=()=>rej(tx.error)});try{writePreview(ref,await makePreview(uploadBlob))}catch(e){}return ref}catch(e){return await makePreview(uploadBlob)}
  }
  ...
  document.addEventListener("visibilitychange",()=>{if(!document.hidden){refresh(); if(SUPA_ENABLED) remoteLoad(); window.dispatchEvent(new Event("rentHubDataChanged"))}});
  window.addEventListener('online',()=>{if(SUPA_ENABLED){remoteLoad();scheduleRemoteSave(100)}});
