...
const Store = (() => {
  ...
  const SUPA_KEY = CFG.anonKey || CFG.publishableKey || '';
  if (!SUPA_URL || !SUPA_KEY) throw new Error('RentHub requires Supabase configuration.');
  ...
  async function req(path,options={}){const controller=new AbortController();const t=setTimeout(()=>controller.abort(),8000);try{const res=await fetch(`${SUPA_URL}${path}`,{...options,signal:controller.signal,headers:{...HEADERS,...(options.headers||{})}});const txt=await res.text();let body=null;try{body=txt?JSON.parse(txt):null}catch{body=txt}if(!res.ok)throw new Error(body?.message||body?.hint||body?.error||`Supabase HTTP ${res.status}`);return body}finally{clearTimeout(t)}}
  async function loadRemote(){const rows=await req(`/rest/v1/${TABLE}?id=eq.${encodeURIComponent(ID)}&select=data,updated_at`);if(!rows.length){data=normalize(seed());await saveRemote();}else data=normalize(rows[0].data);remoteReady=true;window.dispatchEvent(new Event('rentHubDataChanged'));return data}
  ...
  const ready=(async()=>{await loadRemote();await validateSession();decorateLinks();return true})().catch(e=>{remoteReady=false;console.error('RentHub requires Supabase:',e);document.body.innerHTML=`<main style="padding:40px;font-family:system-ui"><h1>Supabase connection required</h1><p>${esc(e.message)}</p><p>No browser data is being used. Check your Supabase URL, key, RLS policies and SQL setup.</p></main>`;throw e});
  return {get data(){return data},save,log,uid,escapeHtml:esc,money,refresh,getRoom,getProperty,sync:()=>syncPropertyCompatibility(data),putImage,getImage:async()=>null,deleteImage,hydrateImages,releaseObjectUrls,initRemote:loadRemote,ready,remoteEnabled:()=>true,remoteStatus:()=>({configured:true,connected:remoteReady}),DEFAULT_IMG,isLoggedIn,login,logout,isLandlordLoggedIn,currentLandlord,loginLandlord,logoutLandlord,isAgentLoggedIn,currentAgent,loginAgent,logoutAgent,sessionToken:()=>sessionToken,decorateLinks};
})();
